export default function Hero({ apartment }) {
  ...
  return (
    <section className="relative overflow-hidden pt-16">
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-10 md:py-28">
        <div className="max-w-2xl rounded-[28px] border border-white/10 bg-[#07131f]/48 p-5 shadow-2xl shadow-black/30 backdrop-blur-[2px] md:p-9">
          ...
          <h1 className="mt-4 whitespace-pre-line text-3xl font-black leading-[1.18] tracking-[-0.055em] text-white md:mt-5 md:text-6xl">
            {lines.map((line, idx) => (
              <span key={idx}>
                {line}
          ...
          <p className="mt-4 max-w-xl text-sm font-medium leading-relaxed text-white/90 md:mt-5 md:text-lg">
            월 렌탈료부터 약정·관리 방식·설치 가능일까지,<br className="hidden md:block" /> 우리 집 조건에 맞는 렌탈 구성을 한 번에 비교해보세요.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row md:mt-8">
            <a
              href="#consult"
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#FEE500] px-6 py-3.5 text-base font-extrabold text-[#191919] shadow-lg shadow-black/30 transition hover:-translate-y-0.5 hover:bg-[#ffef42] md:py-4"
            >
              내 입주 조건으로 견적 보기
            ...
            <a
              href="#benefits"
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/35 bg-white/5 px-5 py-3.5 text-base font-bold text-white transition hover:bg-white/15 md:py-4"
            >
              비교 가능한 제품 보기
              <span aria-hidden="true">→</span>
            </a>
          </div>
          <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-white/90 md:mt-7">
